...
export const StudyPlannerCard: React.FC = () => {
  ...
  const handleSavePlan = async () => {
    try {
      ...
      toast({
        title: "Plano salvo!",
        description: "Bons estudos nesta semana."
      });
      telemetry.trackUserAction("save_plan", "study_planner", {
        goals: goals.length,
  ...
  const handleToggle = async (idx: number) => {
    try {
      ...
      telemetry.trackUserAction("toggle_task", "study_planner", {
        index: idx
      });
    } catch (e: any) {
      toast({
  ...
  const handleSaveReflection = async () => {
    try {
      ...
      toast({
        ...
        description: "Excelente hábito metacognitivo!"
      });
      telemetry.trackUserAction("save_reflection", "study_planner", {
        rating
      });
    } catch (e: any) {
      toast({
  ...
  return <Card className="card-gradient px-0 my-0">
      <CardHeader>
      ...
      <CardContent className="space-y-6 mx-0">
        <div>
          ...
          <Textarea value={goalsText} onChange={e => setGoalsText(e.target.value)} placeholder="Ex: Resolver 5 problemas de frações\nEstudar 20 min por dia" />
        </div>
        ...
        <div className="space-y-3">
          ...
          {tasks.map((t, i) => <div key={i} className="flex items-center gap-3">
        ...
        <div className="space-y-2">
          ...
          <Textarea value={reflection} onChange={e => setReflection(e.target.value)} placeholder="O que funcionou? Onde tive dificuldade? Como vou ajustar?" />
          <div className="flex items-center gap-3 mt-2">
            <label className="text-sm">Autoavaliação (1-5)</label>
            <Input type="number" className="w-20" min={1} max={5} value={rating ?? ""} onChange={e => setRating(e.target.value ? Number(e.target.value) : null)} />
