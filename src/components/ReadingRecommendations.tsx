...
import { BookOpen, Heart, Star, Bookmark, CheckCircle, Lightbulb, Rocket, DollarSign, Users, ArrowRight } from "lucide-react";
...
const ReadingRecommendations = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const {
        data,
        error
      } = await supabase.from('user_books').select('book_id, is_favorite, has_read').eq('user_id', user.id);
      if (error) {
  ...
  const explorationIdeas = [{
    ...
    content: ['Moziah Bridges - Aos 9 anos criou uma empresa de gravatas e hoje vende para lojas famosas!', 'Mikaila Ulmer - Começou vendendo limonada aos 4 anos e hoje tem uma empresa de bebidas naturais.', 'Ryan Kaji - Criou um canal no YouTube aos 3 anos e hoje é um dos youtubers mais ricos do mundo!']
  }, {
    id: 'product-journey',
    title: '🏭 Como um Produto é Feito',
    description: 'Do campo à caixinha de suco - entenda a cadeia de produção e criação de valor.',
    content: ['1. A ideia nasce: alguém percebe uma necessidade', '2. Pesquisa e desenvolvimento: testam a ideia', '3. Produção: fabricam o produto', '4. Distribuição: levam até as lojas', '5. Venda: chega até você!']
  }, {
    id: 'discussion-idea',
    title: '🧠 Ideia para Discussão',
    description: 'Pense no seu brinquedo favorito e reflita sobre sua criação.',
    content: ['Quem teve a ideia desse brinquedo?', 'Como essa pessoa transformou a ideia em realidade?', 'Que problemas esse brinquedo resolve?', 'Qual ideia você teria para deixar outras crianças mais felizes?']
  }];
  const toggleFavorite = async (bookId: string) => {
    const current = books.find(b => b.id === bookId);
    const next = !current?.isFavorite;
    setBooks(books.map(book => book.id === bookId ? {
      ...book,
      isFavorite: next
    } : book));
    if (!user) return;
  ...
  const toggleRead = async (bookId: string) => {
    ...
    if (error) {
      ...
      toast({
        title: 'Não foi possível atualizar leitura',
        description: 'Tente novamente.'
      });
    }
  };
  return <div className="min-h-screen bg-gradient-subtle">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-primary mb-4 bg-lime-700">
        ...
        <div className="space-y-8 mb-8">
          ...
          <Card className="border-primary/20">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {books.filter(book => book.tags.some(tag => tag.includes('empreendedorismo') || tag.includes('negócios') || tag.includes('protagonismo'))).map(book => <Card key={book.id} className="overflow-hidden shadow-card hover:shadow-lg transition-shadow border-primary/10">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="flex gap-2 ml-4">
                          <Button variant="ghost" size="sm" onClick={() => toggleFavorite(book.id)} className="p-2">
                            <Heart className={`w-4 h-4 ${book.isFavorite ? 'fill-red-500 text-red-500' : 'text-muted-foreground'}`} />
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => toggleRead(book.id)} className="p-2">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex gap-2 flex-wrap">
                          ...
                          {book.tags.map(tag => <Badge key={tag} variant="secondary" className="text-xs bg-primary/10 text-primary">{tag}</Badge>)}
                        </div>
                      </div>
                    </CardContent>
                  </Card>)}
          ...
          <Card className="border-primary/20">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {books.filter(book => book.tags.some(tag => tag.includes('mulheres') || tag.includes('inspiração') || tag.includes('liderança'))).map(book => <Card key={book.id} className="overflow-hidden shadow-card hover:shadow-lg transition-shadow border-primary/10">
                    <div className={`h-4 bg-gradient-to-r ${book.color}`}></div>
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
          ...
          <Card className="border-primary/20">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {books.filter(book => book.tags.some(tag => tag.includes('dinheiro') || tag.includes('poupança') || tag.includes('investimento'))).map(book => <Card key={book.id} className="overflow-hidden shadow-card hover:shadow-lg transition-shadow border-primary/10">
                    <div className={`h-4 bg-gradient-to-r ${book.color}`}></div>
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
          ...
          <Card className="border-primary/20">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {books.filter(book => book.tags.some(tag => tag.includes('emoções') || tag.includes('inteligência-emocional') || tag.includes('autoconhecimento') || tag.includes('paciência') || tag.includes('gestão-emocional') || tag.includes('jogos') || tag.includes('expressão'))).map(book => <Card key={book.id} className="overflow-hidden shadow-card hover:shadow-lg transition-shadow border-primary/10">
                    <div className={`h-4 bg-gradient-to-r ${book.color}`}></div>
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
        ...
        <Card className="border-primary/20">
          ...
          <CardContent>
            <div className="space-y-4">
              {explorationIdeas.map(idea => <Card key={idea.id} className={`cursor-pointer transition-all hover:shadow-md border-primary/10 ${selectedIdea === idea.id ? 'border-primary shadow-md' : ''}`} onClick={() => setSelectedIdea(selectedIdea === idea.id ? null : idea.id)}>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <ArrowRight className={`w-5 h-5 text-primary transition-transform ${selectedIdea === idea.id ? 'rotate-90' : ''}`} />
                    </div>
                    
                    {selectedIdea === idea.id && <div className="mt-4 pt-4 border-t border-primary/20">
                        <div className="space-y-2">
                          {idea.content.map((item, index) => <div key={index} className="flex items-start gap-2">
