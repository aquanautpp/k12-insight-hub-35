-- Emotional Check-ins MVP (retry without IF NOT EXISTS on policies)
...
do $$ begin
  begin
    create policy "Users can view their own ei_checkins"
      on public.ei_checkins for select
      using (auth.uid() = user_id);
  exception when duplicate_object then null; end;

  begin
    create policy "Users can insert their own ei_checkins"
