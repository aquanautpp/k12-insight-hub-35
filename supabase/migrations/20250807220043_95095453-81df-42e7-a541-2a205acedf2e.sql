...
create policy if not exists "Users can view their own study_cycles"
  on public.study_cycles for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own study_cycles"
  on public.study_cycles for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own study_cycles"
...
create policy if not exists "Users can view their own review_items"
  on public.review_items for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own review_items"
  on public.review_items for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own review_items"
